...
bot.on("message", async (message) => {
    ...
    if(cmd === `${prefix}help`){
        ...
        .setColor("RANDOM")
        .setTitle("Parancsok:")
        .addField("⚒️Moderáció:⚒️", "\`?ban\`, \`?unban\`, \`?kick\`, \`?mute\`, \`?tempmute\`, \`?unmute\`, \`?warn\`, \`?report\`, \`?clear\`" )
        .addField("❗Infók:❗", "\`?uptime\`, \`?botinfo\`, \`?help\`")
        .addField("💸Economy:💸", "\`?bal\`, \`?daily\`, \`?bolt\`, \`?lb\`, \`?pay\`, \`?work\`, \`?slot\`")
        .addField("🤣Szórakozás:🤣","\`?meme\`, \`?cica\`, \`?kutya\`, \`?ship\`, \`?számológép\`, \`?8ball\`, \`?hack\`")
        .addField("❓Érdekességek:❓", "\`?weather\`, \`?szavazás\`, \`?szia\`, \`?este\`, \`?reggel\`, \`?nappal\`, \`?alkohol\`, \`?drog\`")
        .setThumbnail(message.author.displayAvatarURL())
        .setDescription("Ha megkarasz tudni egy parancsról több mindent, használd ezt: ?help<parancs neve>, fontos, hogy egybeírd! Minden parancs elé egy ?-et tegyél!")
        .setFooter("[HUN] Gamer Community | Készítő: Son of Fury")
        .setTimestamp()
    ...
    if(cmd === `${prefix}alkohol`){
        ...
        .addField("☠️Tönkreteszi a tüdőt!☠️", "Csak jó tanács!")
        .setThumbnail(message.author.displayAvatarURL())
        .setFooter("[HUN] Gamer Community | Készítő: Son of Fury")
        .setTimestamp()
    ...
    if(cmd === `${prefix}drog`){
        ...
        .addField("☠️Tönkreteszi a szervezetedet!☠️", "Csak jó tanács")
        .setThumbnail(message.author.displayAvatarURL())
        .setFooter("[HUN] Gamer Community | Készítő: Son of Fury")
        .setTimestamp()
